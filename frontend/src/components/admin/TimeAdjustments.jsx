...
export default function AdminTimeAdjustments() {
  ...
  const exportToCsv = () => {
    const filteredRows = filterRowsTimeAttendance(
      rows,
      selectStartDate,
      selectEndDate,
      searchRfid
    ).filter((row) => !selectedSite || row.cloud_site === selectedSite)
      .map(({ reasons, isCico, filePath, cicoId, startDateWithNoTime, endDateWithNoTime, ...row }) => row); // Remove filepath from the exported CSV
  ...
  const handleRowClick = (rowId) => {
    navigate(`/admin-view-time-adjustment-details/${rowId}`);
  };
  ...
  const fetchTimeAdjustments = async () => {
    try {
      const response = await fetch(
        `http://${HOST}:${BACKEND_PORT}/api/get-new-time-adjustments`
      );
        ...
        <ThemeProvider theme={lightTheme}>
          ...
          >
            <Paper elevation={2} sx={{ width: "100%" }}>
              ...
              <TableContainer>
                <Table sx={{ minWidth: 2000 }} aria-labelledby="tableTitle">
                  <EnhancedTableHead
                  ...
                  <TableBody>
                    {Array.isArray(rows) &&
                      filterRowsTimeAttendance(
                        ...
                        .filter(row => !selectedSite || row.cloud_site === selectedSite) // Filter rows based on selected site
                        .slice(
                          page * rowsPerPage,
                        ...
                        .map((row, index) => {
                          return (
                            ...
                            >
                              ...
                              >
                                {row.traineeRemark ? row.traineeRemark : "-"}
                              </TableCell>
                              <TableCell
                                align="center"
                                sx={{
